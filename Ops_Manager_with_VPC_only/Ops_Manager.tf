...
// Connect/SSH into Ops Manager VM and execute below commands.
  provisioner "remote-exec" {
    inline = [
      ...
      "om -t https://${aws_eip_association.eip_association.public_ip}/ -u ${var.Ops_Manager_Username} -p ${var.Ops_Manager_Password} -k configure-director -c director_config.yaml",
      "om -t https://${aws_eip_association.eip_association.public_ip}/ -u ${var.Ops_Manager_Username} -p ${var.Ops_Manager_Password} -k curl -p /api/v0/installations -x POST -d '${local.apply_changes_all_tiles}'",
